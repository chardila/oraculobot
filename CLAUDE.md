...
**football-data.org free plan** (Tier 1) supports only the 13 fixed competitions listed below — friendly matches, qualifiers, and any other competition return no data. For this project only `WC` (FIFA World Cup) is used. Supported endpoints per competition:
- `GET /v4/competitions/WC/matches?season=2026&status=FINISHED` — match results with score (fullTime, halfTime), status, stage, group, referees. Score object includes `winner`, `duration` (REGULAR/EXTRA_TIME/PENALTY_SHOOTOUT), `fullTime`, `halfTime`. **No lineups** — the `/v4/matches/{id}/lineups` endpoint returns 404.
...
- `GET /v4/competitions/WC/teams?season=2026` — all 48 teams with full squad (player name, position, dateOfBirth, nationality) and coach.
- `GET /v4/competitions/WC/scorers?season=2026` — top scorers (populated once matches are played).
- `GET /v4/matches/{id}/head2head` — historical head-to-head record between the two teams (Tier 1 only, limited to prior WC meetings).
- Supported competitions: `WC`, `PL`, `CL`, `BL1`, `SA`, `PD`, `FL1`, `DED`, `PPL`, `EC`, `BSA`, `ELC`, `CLI`.
- **Rate limit: 10 requests/minute** (headers: `x-requests-available-minute`, `x-requestcounter-reset: 60`). The check-results workflow runs every 30 min and makes 1 + N calls (1 for FINISHED list, 1 per pending match) — well within limits.
